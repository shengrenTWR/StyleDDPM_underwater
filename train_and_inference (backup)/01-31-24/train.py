...
def get_loss(model, x_0, t):
    ...
    noise_pred = model(x_concatenated, t)    # 所以 u-net 就是想進辦法預測雜訊 （與輸入圖片等大小）
...
def sample_timestep(x, x_0, y, t, model, model2):
    ...
    x_0 = x_0.to(device)
    x_concatenated = torch.cat((x, x_0), dim=1)  # 這邊concatenate 不起來的原因是因為batch size 是 5 不是一張
    ...
    # Mean for Y   # 卡在這裡, 主要是要把x換成 y 可是如果輸出已經是 y 那得到的新輸入 x 就是 y 那就要雙輸出
